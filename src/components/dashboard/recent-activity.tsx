...
export function RecentActivity({ logs }: RecentActivityProps) {
  ...
  return (
    <div className="rounded-3xl border border-slate-200/80 dark:border-[#1e2f4d]/80 bg-white dark:bg-[#0e192d] p-6 sm:p-7 shadow-subtle">
      <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-800 mb-5">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 font-sans">Recent Board & Registry Activity</h3>
            <p className="text-xs text-slate-500 dark:text-slate-400">Official chronological audit log trail</p>
          </div>
        </div>
          ...
          {logs.slice(0, 7).map((log) => {
            ...
            return (
              <div key={log.id} className="relative flex items-start gap-3.5 group">
                ...
                {/* Content Card */}
                <div className="flex-1 p-3.5 rounded-2xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-[#091424]/70 hover:bg-white dark:hover:bg-[#13233f] hover:border-slate-200 dark:hover:border-teal-500/30 hover:shadow-subtle transition-all duration-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-xs text-slate-900 dark:text-slate-100">{log.user_name}</span>
                        <span className={`text-[10px] font-bold px-1.5 py-0.2 rounded-full ${meta.tagColor}`}>
                          {meta.tag}
                        </span>
                      </div>
                  ...
                  <span className="text-[11px] text-slate-400 shrink-0 font-mono self-end sm:self-center">
