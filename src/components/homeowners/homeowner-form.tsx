...
export function HomeownerForm({ initialData, onSubmit, isEditing = false }: HomeownerFormProps) {
  ...
  const addMemberRow = () => {
    ...
    setHouseholdCount((prev) => Math.max(prev, members.length + 2));
  };
      ...
      <div className="rounded-3xl border border-slate-200/80 dark:border-[#1e2f4d] bg-white dark:bg-[#0e192d] p-6 sm:p-8 shadow-subtle space-y-6">
        ...
        <div className="max-w-xs">
          <Input
            ...
            helperText="Total headcount including the principal owner"
          />
        </div>
        ...
        <div className="space-y-3 pt-1">
          {members.length === 0 ? (
            ...
            members.map((member, index) => (
              <div
                key={index}
                className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4 rounded-2xl border border-slate-200 dark:border-[#1e2f4d] bg-slate-50/60 dark:bg-[#0a1526] shadow-xs hover:border-slate-300 dark:hover:border-slate-600 transition-all animate-fade-in"
              >
